  ...
  const handleExportPDF = () => {
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          <div class="header"><h1>Financial Report</h1><p>Event Ledger for ${user?.name}</p></div>
          <table>
            <thead><tr><th>Date</th><th>Description</th><th>Category</th><th>Amount</th></tr></thead>
            <tbody>${rowsHtml}</tbody>
          </table>
          <div class="summary">
            <p><b>Total Income:</b> $${stats.income.toLocaleString()}</p>
            <p><b>Total Expenses:</b> $${stats.expenses.toLocaleString()}</p>
            <p><b>Net Balance:</b> $${stats.balance.toLocaleString()}</p>
          </div>
        </body>
      </html>
    `);
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">Real-time expenditure monitoring</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={refreshFinance} className="p-3.5 bg-white border border-slate-200 rounded-2xl text-slate-500 hover:text-amber-500 transition-all cursor-pointer">
            ...
            <tbody className="divide-y divide-slate-50">
              {entries.length > 0 ? entries.map(entry => (
                <tr key={entry.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-6 py-4 text-sm text-slate-500 font-medium">{entry.date}</td>
                  <td className="px-6 py-4"><p className="text-sm font-bold text-slate-800">{entry.description}</p></td>
                  <td className="px-6 py-4"><span className="px-2 py-1 bg-slate-100 rounded-lg text-[10px] font-bold text-slate-500 uppercase tracking-tight">{entry.category}</span></td>
                  <td className={`px-6 py-4 text-sm font-black ${entry.type === 'Income' ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {entry.type === 'Income' ? '+' : '-'}${entry.amount.toLocaleString()}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => handleDelete(entry.id)} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                  </td>
                </tr>
              )) : <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-400 text-sm italic">No records found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-[32px] p-8 shadow-2xl animate-in zoom-in-95">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-2xl font-bold text-[#0f172a]">New Ledger Entry</h2>
            ...
            <form onSubmit={handleAddEntry} className="space-y-6">
              <div>
                ...
                <input required className="w-full px-5 py-4 bg-slate-50 border border-slate-100 focus:border-amber-500 rounded-2xl outline-none" value={formData.description} onChange={e => setFormData(p => ({ ...p, description: e.target.value }))} />
              </div>
              ...
              <button disabled={isLoading} className="w-full py-4.5 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-200 text-white font-bold rounded-2xl shadow-xl transition-all active:scale-95 cursor-pointer">
                {isLoading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : 'Confirm Transaction'}
              </button>
            </form>
          </div>
        </div>
...
const FinanceStat = ({ icon, label, value, color }: any) => (
  <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm group hover:border-slate-300 transition-all">
    ...
    <h3 className={`text-2xl font-black tracking-tight ${color}`}>${value.toLocaleString()}</h3>
  </div>
);